...
4. Add the appropriate methods to the `EVRA_Hand` input events:
    1. Add `StartDestinationSelection()` to a button input event (typically `Index Trigger Down`).
...
Currently, there are **3** ways that `EVRA_Locomotion` supports:
...
Prior to any of these methos, a locomotion destination must be selected via `EVRA_Pointer` and `EVRA_LocCursor`. The locomotion will **not** occur if:
