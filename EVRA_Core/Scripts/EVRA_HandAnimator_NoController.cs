...
public class EVRA_HandAnimator_NoController : MonoBehaviour
{
    [SerializeField]
    ...
    // Index Finger Animations
    private string  indexTriggerEventName = "Index Trigger Animation", indexAnimatorParam = "indexFinger",
                    touchIndexTriggerEventName = "Touch Index Trigger Animation", touchIndexAnimatorParam = "touchIndexFinger";
    private float   indexCurrent, indexTarget, 
                    touchIndexCurrent, touchIndexTarget;

    // Grip Fingers Animations
    private string  gripTriggerEventName = "Grip Trigger Animation",
                    gripAnimatorParam = "gripFingers";
    private float   gripCurrent, gripTarget;
    
    // Thumb Animations
    private string thumbAnimatorParam = "thumbFinger";
    /*
    ...
    */
    ...
    public void Init(EVRA_Hand hand) {
        ...
        if (m_hand.ovrControllerHelper != null) {
            // There's a controller - we need to delete ourselves
            gameObject.SetActive(false);
            return;
        }
        AddNoControllerAnimations();
    }

    void AddNoControllerAnimations() {
         // Index Trigger
        m_hand.AddNewEvent(
        ...
        m_hand.AddNewEvent(
            touchIndexTriggerEventName,
            new InputEventMap(InputType.TouchIndexTrigger, InputEvent.Any),
    ...
    void UpdateTouchIndex(InputEventDataPackage package) {
        touchIndexTarget = package.inputs[InputType.TouchIndexTrigger].response.direction.x;
    }
    ...
    void UpdateThumbstick(InputEventDataPackage package) {
        ...
        thumbstickXTarget = Remap(dir.x, -1f, 1f, 0f, 1f);
        thumbstickYTarget = Remap(dir.y, -1f, 1f, 0f, 1f);
    ...
    */
    ...
    void AnimateGripTrigger() {
        if (gripCurrent != gripTarget) {
            ...
            m_animator.SetFloat(gripAnimatorParam, gripCurrent);
            // For now, make the thumb follow the grip
            m_animator.SetFloat(thumbAnimatorParam, gripCurrent);
        }
    }
    void AnimateIndexTrigger() {
        ...
        if (touchIndexCurrent != touchIndexTarget) {
            touchIndexCurrent = Mathf.MoveTowards(touchIndexCurrent, touchIndexTarget, Time.deltaTime * m_animationSpeed);
            m_animator.SetFloat(touchIndexAnimatorParam, touchIndexCurrent);
        }
    }
